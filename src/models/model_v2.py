...
class Generator:
    ...
    def dense_unit(self, no_of_filters, f_size, name=None):
        ...
        result.add(tf.keras.layers.Conv3D(no_of_filters, f_size, kernel_initializer=self.WEIGHTS_INIT,
                                          use_bias = self.UTILIZE_BIAS, padding='same'))
        return result
    ...
    def dense_block(self, inputs):
        ...
        dense_unit_output = tf.keras.layers.Concatenate()(dense_units_output)
    ...
    def create_generator(self):
        ...
        dense_block_output = tf.keras.layers.Concatenate()(dense_blocks_output)
        compressor_output = tf.keras.layers.Conv3D(self.compressor_filter_number, self.compressor_filter_size, kernel_initializer=self.WEIGHTS_INIT, 
                                       use_bias=self.UTILIZE_BIAS, padding='same')(dense_block_output)
        ...
        reconstruction_output = tf.keras.layers.Conv3D(1, 1, kernel_initializer=self.WEIGHTS_INIT, 
                                       use_bias=self.UTILIZE_BIAS, padding='same', dtype='float32')(compressor_output)
        final_output = tf.keras.layers.Add()([reconstruction_output, tf.keras.layers.Lambda(lambda x: tf.cast(x, tf.float32))(inputs)])
...
class Model3DRDN:
    ...
    def __init__(self, PATCH_SIZE=40, DB=3, DU=4, BATCH_SIZE=6, LR_G=1e-4, LR_D=1e-4, LAMBDA_ADV=0.01,
     LAMBDA_GRD_PEN=10, MODEL="3DRDN", CRIT_ITER=3, TRAIN_ONLY=''):
